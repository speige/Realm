...
Output ONLY the exact category name";
	...
	public static class Utils
	{
		...
		public static bool RemoveRealmMetadata(string filePath) => RealmMetadataHelper.RemoveMetadata(filePath);
	}
	...
	public static class RealmMetadataHelper
	{
		private static string? FindCliPath()
		{
			string[] candidates = new[]
			{
				...
				Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..\\..\\..\\..\\Generation\\Realm.Tools.Cli.exe")),
				Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "Generation\\Realm.Tools.Cli.exe")),
				Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..\\..\\..\\Realm.Tools.Cli\\bin\\Release\\net10.0\\win-x64\\publish\\Realm.Tools.Cli.exe")),
				Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..\\..\\..\\Realm.Tools.Cli\\bin\\Debug\\net10.0\\win-x64\\publish\\Realm.Tools.Cli.exe")),
				Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..\\..\\..\\..\\Realm\\Realm.Tools.Cli\\bin\\Release\\net10.0\\win-x64\\publish\\Realm.Tools.Cli.exe")),
				Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..\\..\\..\\..\\Realm\\Realm.Tools.Cli\\bin\\Debug\\net10.0\\win-x64\\publish\\Realm.Tools.Cli.exe"))
			};
		...
		public static string? ExtractMetadata(string filePath)
		{
			...
			if (cliPath == null)
			{
				Console.Error.WriteLine("[WARNING] Realm.Tools.Cli.exe not found for ExtractMetadata.");
				return null;
			}

			string tempOutput = Path.Combine(Path.GetTempPath(), $"realm_meta_extract_{Guid.NewGuid():N}.json");
			try
			{
				var psi = new ProcessStartInfo
				{
					FileName = cliPath,
					Arguments = $"metadata_read -i \"{filePath}\" -o \"{tempOutput}\"",
					UseShellExecute = false,
					RedirectStandardOutput = true,
					RedirectStandardError = true,
					CreateNoWindow = true
			...
			{
				Console.Error.WriteLine($"[ERROR] Failed to run CLI metadata_read for '{filePath}': {ex.Message}");
				return null;
			}
			finally
			{
		...
		public static bool AddMetadata(string filePath, string realmMetadataJson)
		{
			...
			if (cliPath == null)
			{
				Console.Error.WriteLine("[WARNING] Realm.Tools.Cli.exe not found for AddMetadata.");
				return false;
			}

			string tempJson = Path.Combine(Path.GetTempPath(), $"realm_meta_add_{Guid.NewGuid():N}.json");
			try
			{
				File.WriteAllText(tempJson, realmMetadataJson);

				var psi = new ProcessStartInfo
				{
					FileName = cliPath,
					Arguments = $"metadata_add -i \"{filePath}\" -d \"{tempJson}\"",
					UseShellExecute = false,
					RedirectStandardOutput = true,
					RedirectStandardError = true,
					CreateNoWindow = true
			...
			{
				Console.Error.WriteLine($"[ERROR] Failed to run CLI metadata_add for '{filePath}': {ex.Message}");
				return false;
			}
			finally
			{
		...
		public static bool RemoveMetadata(string filePath)
		{
			...
			if (cliPath == null)
			{
				Console.Error.WriteLine("[WARNING] Realm.Tools.Cli.exe not found for RemoveMetadata.");
				return false;
			}
			...
			try
			{
				...
				{
					FileName = cliPath,
					Arguments = $"metadata_remove -i \"{filePath}\"",
					UseShellExecute = false,
					RedirectStandardOutput = true,
					RedirectStandardError = true,
					CreateNoWindow = true
			...
			{
				Console.Error.WriteLine($"[ERROR] Failed to run CLI metadata_remove for '{filePath}': {ex.Message}");
				return false;
			}
		}

		public static string? ExtractMetadataFromGlb(string filePath) => ExtractMetadata(filePath);
		public static void AddMetadataToGlb(string filePath, string realmMetadataJson) => AddMetadata(filePath, realmMetadataJson);
		public static void RemoveMetadataFromGlb(string filePath) => RemoveMetadata(filePath);

		public static string? ExtractMetadataFromPng(string filePath) => ExtractMetadata(filePath);
		public static void AddMetadataToPng(string filePath, string realmMetadataJson) => AddMetadata(filePath, realmMetadataJson);
		public static void RemoveMetadataFromPng(string filePath) => RemoveMetadata(filePath);

		public static string? ExtractMetadataFromKtx2(string filePath) => ExtractMetadata(filePath);
		public static void AddMetadataToKtx2(string filePath, string realmMetadataJson) => AddMetadata(filePath, realmMetadataJson);
		public static void RemoveMetadataFromKtx2(string filePath) => RemoveMetadata(filePath);

		public static string? ExtractMetadataFromRanim(string filePath) => ExtractMetadata(filePath);
		public static void AddMetadataToRanim(string filePath, string realmMetadataJson) => AddMetadata(filePath, realmMetadataJson);
		public static void RemoveMetadataFromRanim(string filePath) => RemoveMetadata(filePath);

		public static string? ExtractMetadataFromOgg(string filePath) => ExtractMetadata(filePath);
		public static void AddMetadataToOgg(string filePath, string realmMetadataJson) => AddMetadata(filePath, realmMetadataJson);
		public static void RemoveMetadataFromOgg(string filePath) => RemoveMetadata(filePath);
	}
}
