...
public class HumanoidBoneMapper
{
	...
	public static int FindBoneInSkeleton(Skeleton3D skeleton, HumanoidBone canonicalBone)
	{
		...
		for (int i = 0; i < boneCount; i++)
		{
			...
			if (TryMapToCanonical(boneName, out var mapped) && mapped == canonicalBone)
			{
				return i;
			}
		}
	...
	public static Dictionary<HumanoidBone, int> BuildSkeletonBoneMap(Skeleton3D skeleton)
	{
		...
		{
			string boneName = skeleton.GetBoneName(i);
			if (TryMapToCanonical(boneName, out var mapped))
			{
