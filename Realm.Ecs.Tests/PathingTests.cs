...
namespace Realm.Ecs.Tests
{
    ...
    public class PathingTests
    {
        ...
        [Test]
        public void TestDynamicCarveAndUncarveObstacle()
        {
            InitializeTerrain();
            ref var terrain = ref _world.Get<TerrainState>(_worldEntity);
            _terrainNavMeshService.BakeNavMesh(ref terrain);
            ...
            // Dynamically carve obstacle in the center
            _terrainNavMeshService.CarveObstacle(ref terrain, Vector3.Zero, 3.5f);
            ...
            {
                _movementService.StepMovement(0.1f);
                var p = _world.Get<Position>(unit).Value;
                path.Add(p);
                ticks++;
            }

            Assert.That(_world.Has<MoveTo>(unit), Is.False, "Unit should arrive at destination around dynamically carved obstacle.");
            var finalPos = _world.Get<Position>(unit).Value;
            Assert.That(Vector3.Distance(finalPos, destination), Is.LessThan(1.5f));

            // Verify unit did not walk directly through center of carved obstacle
            foreach (var pt in path)
            {
                if (Math.Abs(pt.X) < 1.5f && Math.Abs(pt.Z) < 1.5f)
                {
                    Assert.Fail($"Unit penetrated carved obstacle at {pt}");
                }
            }

            // Now dynamically uncarve obstacle
            _terrainNavMeshService.UncarveObstacle(ref terrain, Vector3.Zero, 3.5f);

            // Unit moves back directly across center
            _world.Add(unit, new MoveTo(start));
            ...
            Assert.That(_world.Has<MoveTo>(unit), Is.False, "Unit should return to start through uncarved area.");
            float returnLength = 0;
            for (int i = 1; i < returnPath.Count; i++)
            {
                returnLength += Vector3.Distance(returnPath[i - 1], returnPath[i]);
            }
            Assert.That(returnLength, Is.LessThan(45.0f), "Direct return path should be close to straight line (40 units).");
        }
    }
}
